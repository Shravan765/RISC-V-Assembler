...
struct parsed_u_command u_check_register(int instr_space_index, char* command, char* inst){
    ...
    struct parsed_u_command u_command;
    
    u_command.instr = malloc(sizeof(char)*(strlen(inst)+1));
    strcpy(u_command.instr, inst);
    u_command.instr[strlen(inst)] = '\0';
    ...
    char num[12] = "";
    int i = 0;
    if (command[command_counter] == '+' ){ //handling + , -ve numbers are not allowed
        if (command[command_counter+1] -'0' >9 || command[command_counter+1] -'0' <0 ){
            return u_command;//only +/- sign, no number
        }
    ...
    if (command[command_counter] -'0' >9 || command[command_counter] -'0' <0 ){
            return u_command; //there needs to be a number
    }
    ...
    long long val = strtoll(num, NULL, 10);
    //printf("%lld\n", val);
...
void u_command_binary_convert(struct parsed_u_command u_command, char bin[33]){
